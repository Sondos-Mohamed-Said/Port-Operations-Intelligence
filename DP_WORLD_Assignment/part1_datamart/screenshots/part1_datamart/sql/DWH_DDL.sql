...
CREATE TABLE mart.Dim_Container (
    ...
    container_size NVARCHAR(50),                         -- 20, 40, 45
    is_reefer BIT,                              -- 1 for Reefer, 0 for Dry
    --container_type_desc NVARCHAR(20)            -- HC, Standard, etc.
);
GO

ALTER TABLE mart.Dim_Container
ADD CONSTRAINT UQ_Container_Specs UNIQUE (container_no, container_size, is_reefer);
...
CREATE TABLE mart.Dim_Equipment (
    ...
    acquired_date_SK INT,                       -- Foreign Key to Dim_Date
    acquired_time_SK INT,                       -- Foreign Key to Dim_Time
...
INSERT INTO mart.Dim_Direction (direction_name) VALUES ('IN'), ('OUT')
...
CREATE TABLE mart.Fact_Gate_Transactions (
    ...
    gate_txn_id    INT NOT NULL DEFAULT -1,          -- BK from source
